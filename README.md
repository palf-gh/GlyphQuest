...
- **テーマ切替**: 右下の設定ボタンから Forest / Cyber / Moonlight / Candy / Ocean / Y2K テーマを選択
- **スマートフィルタ**: 非出力 Glyph とスマートコンポーネント用 Glyph を除外
- **マスター単位スコア**: 全マスターに shapes があれば 1.0、一部のみの場合は部分点

### テーマ

同梱のデフォルトスキン（Forest / Cyber / Moonlight / Candy / Ocean / Y2K）の UI 画像アセットは、すべて Codex によって生成されたものです。
...
- **主题切换**: 通过右下角设置按钮选择 Forest / Cyber / Moonlight / Candy / Ocean / Y2K 主题
- **智能过滤**: 排除不可导出的字形和智能组件用字形
- **按母版计分**: 所有母版都有 shapes 时计为 1.0，部分完成时按比例计分

### 主题

内置默认皮肤（Forest / Cyber / Moonlight / Candy / Ocean / Y2K）的 UI 图片资源均由 Codex 生成。
...
- **테마 전환**: 오른쪽 아래 설정 버튼에서 Forest / Cyber / Moonlight / Candy / Ocean / Y2K 테마 선택
- **스마트 필터**:내보내지 않는 글리프와 스마트 컴포넌트용 글리프 제외
- **마스터별 점수**: 모든 마스터에 shapes가 있으면 1.0, 일부만 있으면 비율로 계산

### 테마

포함된 기본 스킨(Forest / Cyber / Moonlight / Candy / Ocean / Y2K)의 UI 이미지 에셋은 모두 Codex로 생성되었습니다.
...
- **Theme switching**: Choose Forest / Cyber / Moonlight / Candy / Ocean / Y2K themes from the bottom-right settings button
...
All UI image assets for the bundled default skins (Forest / Cyber / Moonlight / Candy / Ocean / Y2K) were generated by Codex.
